...
config_version = "2.4.0"     # 配置版本，请勿手动修改
...
llm_timeout_seconds = 60     # 单次 LLM 调用最长等待（秒）；受宿主约 30 秒 RPC 上限约束
...
### v2.4.0
- **修复 Docker/Linux 渲染崩溃**（Issue #6）：宿主升级后 `render.html2png()` 不再接受 `timeout_ms` / `rpc_timeout_ms`，改用宿主当前的 `render_timeout_ms` 单参传渲染超时（仍可在 WebUI「图片渲染超时」调整）。
- **修复回复消息张冠李戴**：A 的消息被 B 回复时，宿主会把被引用的原文拼进回复者的文本，导致原话被误记成回复者所说；现改为从消息段（`raw_message`）重建回复者本人的话、丢弃被引用内容。
- **命令秒回 + 后台生成**（并入 PR #7，致谢 @Cplay00）：`/summary`、`/mysummary` 立即返回「已开始生成」，分析与渲染在后台任务执行，绕开宿主对命令处理的约 60 秒硬超时；后台任务在卸载/热重载时统一取消清理。
- **LLM 调用超时**：移除不稳健的 `rpc_timeout_ms` 透传兜底，改为单一的客户端等待上限（`asyncio.wait_for`；对应「LLM 调用超时」配置，受宿主约 30 秒 RPC 上限约束）。

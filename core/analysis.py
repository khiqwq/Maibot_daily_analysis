...
# 每次 LLM 调用尝试覆盖的 RPC 超时（毫秒）。MaiBot 1.0.0-rc.4 起，能力调用可在 args 里
# 用 rpc_timeout_ms 覆盖默认 RPC 超时；放宽到 60 秒给 LLM 更多余量。旧版 SDK 不支持该参数时
# 会自动退回普通调用（见 _generate）。
_LLM_RPC_TIMEOUT_MS = 60000
...
class AnalysisService:
    ...
    def __init__(
        ...
        rpc_timeout_ms: int = _LLM_RPC_TIMEOUT_MS,
    ):
        self.ctx = ctx
        self.logger = ctx.logger
        # 模型任务名（可由插件配置覆盖）。默认 utils=快速模型，确保 30 秒内返回
        self.model = model or _DEFAULT_MODEL_TASK
        # 单次 LLM 调用的 RPC 超时覆盖（毫秒），可由插件配置覆盖（rc.4+ 生效）
        self.rpc_timeout_ms = int(rpc_timeout_ms) if rpc_timeout_ms else _LLM_RPC_TIMEOUT_MS
        ...
        *,
        ...
        try:
            async with self._llm_semaphore:
                result = await self._generate(prompt, temperature, max_tokens)
    ...
    async def _generate(self, prompt: str, temperature: float, max_tokens: int) -> Any:
        """调用 ctx.llm.generate；优先用 rpc_timeout_ms 覆盖 RPC 超时（MaiBot rc.4+），
        旧版 SDK 不接受该参数（TypeError）时自动退回普通调用，保证两端都可用。"""
        try:
